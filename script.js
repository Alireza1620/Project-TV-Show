...
function fetchEpisodesForShow(showId) {
    ...
    return fetch(`https://api.tvmaze.com/shows/${showId}/episodes`)
        .then(response => {
            if (!response.ok) throw new Error(`Failed to fetch episodes for show ID: ${showId}`);
            return response.json();
        })
        .then(episodes => {
...
function renderShowBanners(shows) {
    ...
    shows.forEach((show) => {
        ...
        summary.textContent = show.summary ? show.summary : "No summary available.";
        banner.appendChild(summary);

        const genres = document.createElement("p");
        genres.textContent = `Genres: ${show.genres.join(", ")}`;
        banner.appendChild(genres);
...
function loadEpisodesForShow(showId) {
    fetchEpisodesForShow(showId).then((episodes) => {
        ...
        // Hide the shows listing and show the episodes
        document.querySelector(".banner-container").style.display = "none";
        document.querySelector(".episodes-container").style.display = "flex";
        document.getElementById("controls").style.display = "block";

        // Add navigation back to the shows listing
        const backButton = document.createElement("button");
        backButton.textContent = "Back to Shows";
        backButton.addEventListener("click", () => {
            document.querySelector(".banner-container").style.display = "flex";
            document.querySelector(".episodes-container").style.display = "none";
            document.getElementById("controls").style.display = "none";
        });
        document.getElementById("controls").appendChild(backButton);
    });
}
